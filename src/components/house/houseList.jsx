...
import { useGetHousesQuery, useDeleteHouseMutation } from '../../features/api/apiHousesSlice.js';
...
export default function HouseList() {
    ...
    const handleDelete = (house) => {
        Swal.fire({
            title: `¿Estas seguro que deseas eliminar la casa ${house.zip_code}?`,
            icon: "warning",
            ...
            <table className="table-auto w-full">
                <thead>
                    <tr className="bg-slate-500">
                        ...
                        <th className="px-4 py-2 text-white">zip_code</th>
                        ...
                        <th className="px-4 py-2 text-white">parking</th>
                ...
                <tbody className="bg-gray-200 ">
                    {houses.map(house => (
                        <tr key={house._id}>
                            ...
                            <td className="border-y-2 px-4 py-2 border-indigo-600">{house.parking}</td>
                            <td className="border-y-2 px-4 py-2 border-indigo-600">{house.image}</td>
                            ...
                            <td className="border-y-2 px-4 py-2 border-indigo-600">
                                <img className="size-40 transition-transform duration-1000 transform hover:scale-110 max-w-none"
                                    src={`https://nodejs-tan-xi.vercel.app/${house.avatar}`} /></td>
                            <td className="border-y-2 px-4 py-2 border-indigo-600">
                                <div className="inline-flex rounded-md shadow-sm" role="group">
                                    <Link to={`/house/${house._id}`}
                                        className="px-4 py-2 text-sm font-medium text-gray-900 bg-transparent border border-gray-900 rounded-l-md hover:bg-gray-200 hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:bg-gray-200 focus:text-gray-900 dark:border-white dark:text-white dark:hover:text-gray-200 dark:hover:bg-gray-700 dark:focus:bg-gray-700">
                                        Edit</Link>
                                    <button type="button"
                                        onClick={() => {
                                            handleDelete(house)
                                        }}
                                        className="px-4 py-2 text-sm font-medium text-gray-900 bg-transparent border border-gray-900 rounded-r-md hover:bg-red-400 hover:text-white focus:outline-none focus:ring-2 focus:ring-gray-500 focus:bg-red-400 focus:text-white dark:border-white dark:text-white dark:hover:text-red-400 dark:hover:bg-gray-700 dark:focus:bg-gray-700">
                                        Delete
                                    </button>
                                </div>
                            </td>
